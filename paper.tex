...
\affiliation{
  \department{Dep. de Inform\'atica e Ci\^encia da Computa\c{c}\~ao}
  \institution{Rio de Janeiro State University, Brazil}
  %\country{Brazil}
}
\email{francisco@ime.uerj.br}

\author{Alexandre Sztajnberg}
\affiliation{
  \department{Dep. de Inform\'atica e Ci\^encia da Computa\c{c}\~ao}
  \institution{Rio de Janeiro State University, Brazil}
  %\country{Brazil}
...
Then, we propose to extend \CEU with asynchronous ISRs and discuss how it can
...
In Listing~\ref{lst.ceu}, the input \& output events are external to the
program and are generated by the host language representing the environment.
%
We now propose to extend \CEU with interrupt services routines (ISRs) to take
control of the whole event loop using a safe and energy-efficient approach.

\subsection{Scoped ISRs}
\label{sec.isrs}
...
input high/low PIN_02;
{
    ...
    EIMSK |= (1 << INT0);  // enables pin-02 ints
    EICRA |= (1 << ISC00); // for level changes
}
spawn async/isr [INT0_vect] do
    emit PIN_02({digitalRead(2)} as high/low);
end
\end{lstlisting}
...
\CEU is designed to interoperate seamlessly with C and supports a compatible
type system with self-evident conversion rules and same internal representation
(e.g., \code{high/low} is a boolean, \code{u8} is an unsigned 8-bit value,
etc.).
\CEU also supports inline C between curly braces (ln~3,5) and interpolation to
...
Our drivers still use C, but only a convenient subset (e.g., assignments and
getters \& setters) for low-level port manipulation.
...
the \code{emit} goes into an input queue and does not affect the synchronous
side immediately.
%
The input driver also configures pin 2 to behave as input and to generate
external interrupts on level transitions (ln~11--15).
%
The queue is static and new emissions of an unhandled input override the same
position in the queue.
This semantics is comparable to interrupt flags in microcontrollers and
...
%
Listing~\ref{lst.usart} illustrates a minimum serial communication (USART)
application (ln~15--24) that consumes incoming bytes (ln~19--23) as they arrive
(ln~18).
%
...
speed.
%
The (unsafe) driver ISR (ln~6--13) simply appends incoming bytes to the end of the
buffer (ln~9) and signals the application (ln~11).
%
As a possible race condition, note that the ISR may fire and append a new byte
to the buffer (ln~9) just before the application clears it (ln~23), in which
...
Listing~\ref{lst.usart} may lead to race conditions (e.g., ln~9,23), and \CEU
...
spawn async/isr [USART_RX_vect] do
    rx_buf = rx_buf .. [{UDR0} as byte];  // appends
    if $rx_buf == 1 then
...
\CEU supports \code{code} reactive abstractions (similar to coroutines) that help hiding
